...
namespace Funcs
{
    ...
    EXTERNALS bool IsF32Equal( f32 first, f32 second, f32 epsilon );
    #define _IsF32Equal( first, second, epsilon ) (::fabsf( first - second ) < epsilon)
    EXTERNALS bool IsF64Equal( f64 first, f64 second, f64 epsilon );
    ...
    EXTERNALS bool IsF32NaN( f32 val );
    EXTERNALS bool IsF64NaN( f64 val );
    EXTERNALS bool IsF32Inf( f32 val );
    EXTERNALS bool IsF32InfPos( f32 val );
    EXTERNALS bool IsF32InfNeg( f32 val );
    EXTERNALS bool IsF64Inf( f64 val );
    EXTERNALS bool IsF64InfPos( f64 val );
    EXTERNALS bool IsF64InfNeg( f64 val );
    ...
    EXTERNALS bool MemTest( void *p_mem, byte val, uiw size );  //  true if size is 0
    EXTERNALS bool MemEquals( const void *cp_mem0, const void *cp_mem1, uiw size );  //  true if size is 0
    ...
    template < typename X > bool IsPowerOf2( const X &val )
    {
